...
ALTER TABLE "public"."relay_tx" ALTER COLUMN "status" SET DATA TYPE text;--> statement-breakpoint
DROP TYPE "public"."request_status";--> statement-breakpoint
CREATE TYPE "public"."request_status" AS ENUM('PENDING', 'SUBMITTED', 'CONFIRMED', 'ERROR');--> statement-breakpoint
ALTER TABLE "public"."relay_tx" ALTER COLUMN "status" SET DATA TYPE "public"."request_status" USING "status"::"public"."request_status";