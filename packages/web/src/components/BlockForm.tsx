...
function Form({ onCancel, onSubmit, editorRef }: FormProps) {
	...
	const { handleSubmit, reset, control } = useForm<{ value: string }>();
	useEffect(() => {
			...
			<Controller
				...
				render={({ field }) => (
					<MDX
						...
						className="z-10 overflow-y-auto p-2 focus:border-primary border-dashed border border-transparent grow"
					/>
					// <Editor
					// 	editorRef={editorRef}
